...
class GUIProxy:
            ...
            for r in value:
                ...
                if all(k in r for k in ['x', 'y', 'width', 'height']):
                    self._regions.append({
                        ...
                        'color': r.get('color', '#FFFF00'),  # Default yellow if not specified
    ...
    def update_region_selector(self):
        ...
        self._bot.log(f"Updated region selector with {len(self._regions)} embedded regions")
...
class BotAPI:
    ...
    def _get_current_frame(self):
        ...
        if self._target_hwnd:
            try:
                ...
                if self._debug_mode:
                    self.log(f"Grabbing window frame: ({left}, {top}, {right}, {bottom})")
    ...
    def find_text(self, text, region_index, case_sensitive=False):
        ...
        try:
            ...
            self.log(f"Vision Error: Tesseract check failed. {e}")
            return None

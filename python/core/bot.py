...
class Bot:
    ...
    def __init__(self):
        ...
        self.user32.SetCursorPos.restype = ctypes.c_int # Returns BOOL (non-zero success)
        ...
        self.user32.mouse_event.argtypes = [ctypes.c_uint, ctypes.c_uint, ctypes.c_uint, ctypes.c_uint, ctypes.c_ulong]
    ...
    def _send_input(self, inputs):
        ...
        try:
            ...
            if result != n_inputs:
                ...
                print(f"Bot: SendInput failed! Sent {result}/{n_inputs}. Error code: {err}")
    ...
    def move_to(self, x, y):
        ...
        if ret == 0:
            ...
            err = ctypes.get_last_error()
            if self._debug_mode:
                print(f"Bot: SetCursorPos({x}, {y}) failed. Error: {err}. Trying SendInput...")
    ...
    def click(self, x, y, button='left', modifiers=[]):
        ...
        for mod in modifiers:
            if mod in KEY_MAP:
                self._send_input([Input(type=INPUT_KEYBOARD, ii=Input_I(ki=KeyBdInput(wVk=KEY_MAP[mod], dwFlags=KEYEVENTF_KEYDOWN)))])
        ...
        self._send_input([Input(type=INPUT_MOUSE, ii=Input_I(mi=MouseInput(dwFlags=down)))])
        time.sleep(random.uniform(0.05, 0.1))
        self._send_input([Input(type=INPUT_MOUSE, ii=Input_I(mi=MouseInput(dwFlags=up)))])

        for mod in reversed(modifiers):
            if mod in KEY_MAP:
                self._send_input([Input(type=INPUT_KEYBOARD, ii=Input_I(ki=KeyBdInput(wVk=KEY_MAP[mod], dwFlags=KEYEVENTF_KEYUP)))])

    def type_text(self, text, delay=0.05):
        """Types the given text using unicode characters."""
        for char in text:
            key_input_down = KeyBdInput(wVk=0, wScan=ord(char), dwFlags=KEYEVENTF_UNICODE)
            key_input_up = KeyBdInput(wVk=0, wScan=ord(char), dwFlags=KEYEVENTF_UNICODE | KEYEVENTF_KEYUP)
            self._send_input([Input(type=INPUT_KEYBOARD, ii=Input_I(ki=key_input_down)),
    ...
    def press_key(self, key, modifiers=[]):
        ...
        if key in KEY_MAP:
            vk_code = KEY_MAP[key]
            self._send_input([Input(type=INPUT_KEYBOARD, ii=Input_I(ki=KeyBdInput(wVk=vk_code, dwFlags=KEYEVENTF_KEYDOWN)))])
            time.sleep(random.uniform(0.05, 0.1))
            self._send_input([Input(type=INPUT_KEYBOARD, ii=Input_I(ki=KeyBdInput(wVk=vk_code, dwFlags=KEYEVENTF_KEYUP)))])

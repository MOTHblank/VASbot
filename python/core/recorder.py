...
class ActionRecorder:
        ...
        try:
            ...
            for event in events:
                if not isinstance(event, dict):
                    raise ValueError(
                        "Invalid recording format: expected list of event objects"
                    )
            ...
            if event_type == "mouse_move":
                ...
                if clicked_region_idx is not None:
                    if button == "left":
                        ...
                        lines.append(
                            f"bot.click_region({clicked_region_idx}, '{button}')"
                        )
                else:
                    if button == "left":
    ...
    def export_to_script(
        self, filepath: str, script_name: str = "recorded_script"
    ) -> None:
    ...
    def _execute_event(event: Dict[str, Any], bot: Bot) -> None:
        ...
        if event_type == "mouse_move":
            ...
            bot.click(event["x"], event["y"], event.get("button", "left"))

...
class ScriptRunner:
    ...
    def _generate_code_from_node(self, node_id: str, nodes: dict, adj: dict, visited: set) -> list[str]:
        ...
        if node_type == "start":
            ...
            if 0 in adj.get(node_id, {}):
                next_node_id = adj[node_id][0]
                true_code = self._generate_code_from_node(next_node_id, nodes, adj, visited.copy())
                generated_code.extend([f"    {line}" for line in true_code] if true_code else ["    pass"])
            else:
                generated_code.append("    pass")
            ...
            if 1 in adj.get(node_id, {}):
                generated_code.append("else:")
                next_node_id = adj[node_id][1]
                false_code = self._generate_code_from_node(next_node_id, nodes, adj, visited.copy())
                generated_code.extend([f"    {line}" for line in false_code] if false_code else ["    pass"])
            ...
            if 0 in adj.get(node_id, {}):
                next_node_id = adj[node_id][0]
                loop_code = self._generate_code_from_node(next_node_id, nodes, adj, visited.copy())
                generated_code.extend([f"    {line}" for line in loop_code] if loop_code else ["    pass"])
            else:
                generated_code.append("    pass")

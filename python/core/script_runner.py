...
class ScriptRunner:
    ...
    def _update_script_state(self, new_state):
        ...
        if new_state == "running":
            if hasattr(self.gui, "play_button"):
    ...
    def _execute(self, script_code):
        try:
            ...
            self.log(
                f"Performance: {self._actions_count} actions, {self._errors_count} errors"
            )
    ...
    def _visual_script_to_code(self, visual_script: dict) -> str:
        ...
        start_node_id = next(
            (n["id"] for n in nodes.values() if n["type"] == "start"), None
        )
        ...
        indented_code = (
            "\n".join([f"    {line}" for line in code_lines if line])
            if code_lines
            else "    pass"
        )
        return f"# Generated from visual script\nwhile bot.is_running:\n{indented_code}\n    bot.wait(0.05) # Prevent CPU-hogging loops"

    def _generate_code_from_node(
        self, node_id: str, nodes: dict, adj: dict, visited: set
    ) -> list[str]:
        ...
        if node_type == "start":
            ...
            if 0 in adj.get(node_id, {}):
                ...
                generated_code.extend(
                    self._generate_code_from_node(next_node_id, nodes, adj, visited)
                )

        elif node_type == "find_click_color":
            # This node type has conditional logic (Success/Failure)
            mods = (
                f", modifiers={params.get('modifiers', [])}"
                if params.get("modifiers")
                else ""
            )
            find_call = (
                f"bot.find_and_click_color(hex_color='{params['hex_color']}', "
            ...
            if 0 in adj.get(node_id, {}):
                ...
                success_code = self._generate_code_from_node(
                    next_node_id, nodes, adj, visited.copy()
                )
                generated_code.extend(
                    [f"    {line}" for line in success_code]
                    if success_code
                    else ["    pass"]
                )
            else:
            ...
            if 1 in adj.get(node_id, {}):
                ...
                failure_code = self._generate_code_from_node(
                    next_node_id, nodes, adj, visited.copy()
                )
                generated_code.extend(
                    [f"    {line}" for line in failure_code]
                    if failure_code
                    else ["    pass"]
                )
            ...
            generated_code.append(
                f"bot.click_region(region_index={params['region_index']}, button='{params['button']}'{mods})"
            )
            if 0 in adj.get(node_id, {}):
                next_node_id = adj[node_id][0]
                generated_code.extend(
            ...
            variable_name = params.get("variable_name", "my_variable")
            variable_value = params.get("variable_value", "0")
            generated_code.append(f"{variable_name} = {variable_value}")
            if 0 in adj.get(node_id, {}):
                next_node_id = adj[node_id][0]
                generated_code.extend(
                    self._generate_code_from_node(next_node_id, nodes, adj, visited)
            ...
            variable_name = params.get("variable_name", "my_variable")
            # This node type is a bit different, it doesn't generate code directly
            # but it can be used as an input to other nodes.
            if 0 in adj.get(node_id, {}):
                next_node_id = adj[node_id][0]
                generated_code.extend(
                    self._generate_code_from_node(next_node_id, nodes, adj, visited)
            ...
            # True branch (output port 0)
            if 0 in adj.get(node_id, {}):
                next_node_id = adj[node_id][0]
                true_code = self._generate_code_from_node(
                    next_node_id, nodes, adj, visited.copy()
                )
                generated_code.extend(
                    [f"    {line}" for line in true_code] if true_code else ["    pass"]
                )
            else:
                generated_code.append("    pass")

            # False branch (output port 1)
            if 1 in adj.get(node_id, {}):
                generated_code.append("else:")
                next_node_id = adj[node_id][1]
                false_code = self._generate_code_from_node(
                    next_node_id, nodes, adj, visited.copy()
                )
                generated_code.extend(
                    [f"    {line}" for line in false_code]
                    if false_code
                    else ["    pass"]
                )
            ...
            condition = params.get("condition", "True")
            generated_code.append(f"while {condition}:")
            # Loop body (output port 0)
            if 0 in adj.get(node_id, {}):
                next_node_id = adj[node_id][0]
                loop_code = self._generate_code_from_node(
                    next_node_id, nodes, adj, visited.copy()
                )
                generated_code.extend(
                    [f"    {line}" for line in loop_code] if loop_code else ["    pass"]
                )
            else:
                generated_code.append("    pass")

            # After loop (output port 1)
            if 1 in adj.get(node_id, {}):
                next_node_id = adj[node_id][1]
                generated_code.extend(
                    self._generate_code_from_node(next_node_id, nodes, adj, visited)
    ...
    def run_script(self, script_code=None, visual_script=None):
        ...
        def counting_log(message):
            ...
            if self._debug_mode and (
                "clicked" in message.lower() or "pressed" in message.lower()
            ):
        ...
        self._message_queue.put(
            ("status", "▶️ Script running... Use hotkeys for control!")
        )
